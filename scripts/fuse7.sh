...
oc create -f https://raw.githubusercontent.com/pszuster/Fuse7TD/master/install/serviceaccount-as-oauthclient-restricted.yml
oc process -f https://raw.githubusercontent.com/pszuster/Fuse7TD/master/install/fuse-ignite-ocp.yml --param=ROUTE_HOSTNAME=fuse-ignite.$fisDomain --param=OPENSHIFT_PROJECT=ignite --param=OPENSHIFT_OAUTH_CLIENT_S
ECRET=$(oc sa get-token syndesis-oauth-client) --param=IMAGE_STREAM_NAMESPACE=openshift | oc create -f -
